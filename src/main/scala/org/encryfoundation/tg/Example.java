...
public final class Example {
    private static Client123 client123 = null;

    private static TdApi.AuthorizationState authorizationState = null;
    private static volatile boolean haveAuthorization = false;
    private static volatile boolean quiting = false;

    private static final Client123.ResultHandler defaultHandler = new DefaultHandler();
    ...
    private static void onAuthorizationStateUpdated(TdApi.AuthorizationState authorizationState) {
        ...
        switch (Example.authorizationState.getConstructor()) {
            ...
            case TdApi.AuthorizationStateWaitEncryptionKey.CONSTRUCTOR:
                client123.send(new TdApi.CheckDatabaseEncryptionKey(), new AuthorizationRequestHandler());
                break;
            case TdApi.AuthorizationStateWaitPhoneNumber.CONSTRUCTOR: {
                String phoneNumber = promptString("Please enter phone number: ");
                client123.send(new TdApi.SetAuthenticationPhoneNumber(phoneNumber, null), new AuthorizationRequestHandler());
                break;
            }
            ...
            case TdApi.AuthorizationStateWaitCode.CONSTRUCTOR: {
                ...
                client123.send(new TdApi.CheckAuthenticationCode(code), new AuthorizationRequestHandler());
                break;
            }
            case TdApi.AuthorizationStateWaitRegistration.CONSTRUCTOR: {
                String firstName = promptString("Please enter your first name: ");
                String lastName = promptString("Please enter your last name: ");
                client123.send(new TdApi.RegisterUser(firstName, lastName), new AuthorizationRequestHandler());
                break;
            }
            case TdApi.AuthorizationStateWaitPassword.CONSTRUCTOR: {
                String password = promptString("Please enter password: ");
                client123.send(new TdApi.CheckAuthenticationPassword(password), new AuthorizationRequestHandler());
                break;
            }
            ...
            case TdApi.AuthorizationStateClosed.CONSTRUCTOR:
                print("Closed");
                if (!quiting) {
                    client123 = Client123.create(new UpdatesHandler(), null, null); // recreate client after previous has closed
                }
                break;
            default:
    ...
    private static void getCommand() {
        ...
        try {
            switch (commands[0]) {
                ...
                case "gc":
                    client123.send(new TdApi.GetChat(getChatId(commands[1])), defaultHandler);
                    break;
                case "me":
                    client123.send(new TdApi.GetMe(), defaultHandler);
                    break;
                case "sm": {
                ...
                case "lo":
                    haveAuthorization = false;
                    client123.send(new TdApi.LogOut(), defaultHandler);
                    break;
                case "q":
                    quiting = true;
                    haveAuthorization = false;
                    client123.send(new TdApi.Close(), defaultHandler);
                    break;
                default:
    ...
    private static void getMainChatList(final int limit) {
        synchronized (mainChatList) {
            if (!haveFullMainChatList && limit > mainChatList.size()) {
                ...
                client123.send(new TdApi.GetChats(new TdApi.ChatListMain(), offsetOrder, offsetChatId, limit - mainChatList.size()), new Client123.ResultHandler() {
                    @Override
    ...
    private static void sendMessage(long chatId, String message) {
        ...
        client123.send(new TdApi.SendMessage(chatId, 0, null, replyMarkup, content), defaultHandler);
    }

    public static void main(String[] args) throws InterruptedException {
        // disable TDLib log
        Client123.execute(new TdApi.SetLogVerbosityLevel(0));
        if (Client123.execute(new TdApi.SetLogStream(new TdApi.LogStreamFile("tdlib.log", 1 << 27))) instanceof TdApi.Error) {
            throw new IOError(new IOException("Write access to the current directory is required"));
        }

        // create client
        client123 = Client123.create(new UpdatesHandler(), null, null);

        // test Client.execute
        defaultHandler.onResult(Client123.execute(new TdApi.GetTextEntities("@telegram /test_command https://telegram.org telegram.me @gif @test")));
    ...
    private static class DefaultHandler implements Client123.ResultHandler {
        @Override
        public void onResult(TdApi.Object object) {
    ...
    private static class UpdatesHandler implements Client123.ResultHandler {
        @Override
        public void onResult(TdApi.Object object) {
            switch (object.getConstructor()) {
    ...
    private static class AuthorizationRequestHandler implements Client123.ResultHandler {
        @Override
        public void onResult(TdApi.Object object) {
            switch (object.getConstructor()) {
                case TdApi.Error.CONSTRUCTOR:
